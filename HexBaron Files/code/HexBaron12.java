...
public class HexBaron {
    ...
    boolean checkSpawnCommandFormat(List<String> items) {
        int result;
        if (items.size() == 2) {
            return checkStandardCommandFormat(items);
        } else if (items.size() == 3) {
            if (!items.get(1).toLowerCase().equals("bomb")) {
                return false;
            }
            try {
...
class BombPiece extends Piece {
    private int movesBeforePrimed = 5;
    ...
    public void destroyPiece(Tile tileLocation) {
        if (!destroyed) {
            connectionsToDestroy = 0;
            movesBeforePrimed = -1;
            destroyed = true;
            List<Tile> listOfNeighbours = new ArrayList<>(tileLocation.getNeighbours());
            for (Tile neighbour : listOfNeighbours) {
    ...
    public Object[] explode(Tile tileLocation) {
        int player1VPs = 0;
        int player2VPs = 0;
        ...
        if (!destroyed) {
            ...
            for (Tile neighbour : listOfNeighbours) {
                ...
                if (tilePiece != null) {
                    ...
                    if (tilePiece.getPieceType().toUpperCase().equals("B")) {
                        baronDestroyed = true;
                    }
...
class HexGrid {
    ...
    private int executeMoveCommand(List<String> items, int fuelAvailable) {
        ...
        for (Tile neighbour : listOfNeighbours) {
            ...
            if (pieceInTile!= null) {
                if (pieceInTile.getPieceType().toUpperCase().equals("X")) {
                    BombPiece bomb = (BombPiece)pieceInTile;
                    if (bomb.primed()) {
                        bomb.destroyPiece(neighbour);
                    }
                }
            }
        }
    ...
    private int executeSpawnCommand(List<String> items, int lumberAvailable, int piecesInSupply) {
        ...
        if (piecesInSupply < 1 || lumberAvailable < spawnCost || !checkTileIndexIsValid(tileToUse)) {
            return -1;
        }
        Piece thePiece = tiles.get(tileToUse).getPieceInTile();
        if (thePiece != null) {
        ...
        if (items.get(1).equals("bomb")) {
            BombPiece newPiece = new BombPiece(player1Turn);
            pieces.add(newPiece);
        ...
        for (Tile t : tiles) {
            if (t.getPieceInTile() != null) {
                ...
                if (noOfConnections >= thePiece.getConnectionsNeededToDestroy()) {
                    if (thePiece.getPieceType().toUpperCase().equals("X")) {
                        BombPiece bomb = (BombPiece)thePiece;
                        Object[] returnObjects = bomb.explode(t);
                        baronDestroyed = (boolean)returnObjects[0];
                        player1VPs += (int)returnObjects[1];
                        player2VPs += (int)returnObjects[2];
                    } else {
                        thePiece.destroyPiece();
