...
public class HexBaron {
    ...
    boolean checkUpgradeCommandFormat(List<String> items) {
        int result;
        if (items.size() == 3) {
            if (!items.get(1).toUpperCase().equals("LESS") && !items.get(1).toUpperCase().equals("PBDS") && !items.get(1).toUpperCase().equals("SCFW")) {
                return false;
            }
            try {
    ...
    boolean checkCommandIsValid(List<String> items, boolean supplyFirst) {
        if (items.size() > 0) {
        switch (items.get(0)) {
                case "move":
    ...
    void playGame(Player player1, Player player2, HexGrid grid) {
        ...
        do {
            ...
            for (int count = 1; count <= 3; count++) {
                ...
                if (items.get(0).toLowerCase().equals("supply") && count == 1) {
                    supplyFirst = true;
                    break;
                }
            }
...
class SCFWPiece extends Piece {
    public SCFWPiece (boolean player1) {
        super(player1);
        pieceType = "F";
        vPValue = 3;
    }
    ...
    public Object[] supply(int fuelAvailable, int lumberAvailable) {
        Object[] returnObjects;
        if ((fuelAvailable >= 5) && (lumberAvailable >= 5)) {
            returnObjects = new Object[]{-1,-5,-5};
        } else {
            returnObjects = new Object[]{0,0,0};
        }
...
class HexGrid {
    ...
    public Object[] executeCommand(List<String> items, int fuelChange, int lumberChange,
        ...
        switch (items.get(0))
        {
            ...
            case "dig":
                ...
                execute = (boolean)returnObjects[0];
                fuelChange = (int)returnObjects[1];
                lumberChange = (int)returnObjects[2];
                if (!execute) {
                ...
                break;
            case "supply":
                Object[] returnObject = executeSupplyCommand(items, fuelAvailable, lumberAvailable);
                execute = (boolean)returnObject[0];
                supplyChange = (int)returnObject[1];
                fuelChange = (int)returnObject[2];
                lumberChange = (int)returnObject[3];
                if (!execute) {
                    return new Object[] {"Couldn't do that", fuelChange, lumberChange, supplyChange};
                }
                break;
    ...
    private Object[] executeSupplyCommand(List<String> items, int fuelAvailable, int lumberAvailable) {
        int tileToUse = Integer.parseInt(items.get(1));
        int fuel = 0;
        int lumber = 0;
        int supply = 0;
        if (checkPieceAndTileAreValid(tileToUse) == false) {
            return new Object[] {false, supply, fuel, lumber};
        }
        Piece thePiece = tiles.get(tileToUse).getPieceInTile();
        if ((thePiece != null) && (thePiece.getPieceType().toUpperCase().equals("F"))) {
            Object[] returnObjects = ((SCFWPiece)thePiece).supply(fuelAvailable, lumberAvailable);
            supply = (int)returnObjects[0];
            fuel = (int)returnObjects[1];
            lumber = (int)returnObjects[2];
            if (supply == -1) {
                return new Object[] {true, supply, fuel, lumber};
            }
        }
    ...
    private int executeUpgradeCommand(List<String> items, int lumberAvailable) {
        ...
        if (!checkPieceAndTileAreValid(tileToUse) || lumberAvailable < 5 || !((items.get(1).equals("pbds") || items.get(1).equals("less") || items.get(1).equals("scfw")))) {
            return -1;
        } else {
            Piece thePiece = tiles.get(tileToUse).getPieceInTile();
            ...
            } else if (items.get(1).equals("less")){
                thePiece = new LESSPiece(player1Turn);
            } else {
                thePiece = new SCFWPiece(player1Turn);
            }
            pieces.add(thePiece);
...
class Player {
    ...
    public void addPieceToSupplyChain() {
        piecesInSupply += 1;
    }
