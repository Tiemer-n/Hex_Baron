...
public class HexBaron {
    ...
    boolean checkUpgradeCommandFormat(List<String> items) {
        int result;
        if (items.size() == 3) {
            if (!items.get(1).toUpperCase().equals("LESS") && !items.get(1).toUpperCase().equals("PBDS") && !items.get(1).toUpperCase().equals("WIZ")) {
                return false;
            }
            try {
...
class WizardPiece extends Piece {
    private boolean teleported;
    
    public WizardPiece(boolean player1) {
        super(player1);
        pieceType = "W";
        vPValue = 3;
        resetTeleport();
    ...
    public int checkMoveIsValid(int distanceBetweenTiles, String startTerrain, String endTerrain) {
        if (distanceBetweenTiles == 1) {
            return fuelCostOfMove;
        } else if (!teleported && (distanceBetweenTiles >= 2) && (distanceBetweenTiles <= 3)) {
            teleported = true;
            return 5;
        }
        return -1;
...
class HexGrid {
    ...
    private int executeUpgradeCommand(List<String> items, int lumberAvailable) {
        ...
        if (!checkPieceAndTileAreValid(tileToUse) || lumberAvailable < 5 || !((items.get(1).equals("pbds") || items.get(1).equals("less") || items.get(1).equals("wiz")))) {
            return -1;
        } else {
            Piece thePiece = tiles.get(tileToUse).getPieceInTile();
    ...
    public void resetWizards() {
        for (Tile t : tiles) {
            Piece thePiece = t.getPieceInTile();
            if (thePiece != null) {
                if (thePiece.getPieceType().toUpperCase().equals("W")) {
                    ((WizardPiece)thePiece).resetTeleport();
                }
            }
        }
    }
