...
class HexGrid {
    ...
    public Object[] executeCommand(List<String> items, int fuelChange, int lumberChange,
        ...
        switch (items.get(0))
        {
            ...
            case "spawn":
                ...
                break;
            case "downgrade":
                lumberCost = executeDowngradeCommand(items);
                if (lumberCost == 0) {
    ...
    private int executeDowngradeCommand(List<String> items) {
        int tileToUse = Integer.parseInt(items.get(1));
        if (!checkPieceAndTileAreValid(tileToUse)) {
            return 0;
        } else {
            Piece thePiece = tiles.get(tileToUse).getPieceInTile();
            if (!thePiece.getPieceType().toUpperCase().equals("P") && !thePiece.getPieceType().toUpperCase().equals("L")) {
                return 0;
            }
